void MagneticLayer::countMagnetoelasticEnergy(FerroelasticSubstrate &FS)
{
	...
	for(int i = 0; i < N; i++)
	{
		for(int j = 0; j < N; j++)
		{
			...
			itsMagnetoelasticEnergy = itsMagnetoelasticEnergy + B1*(Mx*Mx*Deformation[0] + My*My*Deformation[1]) - (B1*B1/(6*E1) + E2/E1*B1*(Deformation[0]+Deformation[1]))*Mz*Mz + (B1*B1/(2*E1)-B2*B2/(2*E4))*(Mx*Mx+My*My)*Mz*Mz + B2*Mx*My*Deformation[2];

			itsMagnetoelasticField[0] = 2*B1*Mx*Deformation[0] + (B1*B1/E1-B2*B2/E4)*Mx*Mz*Mz + B2*My*Deformation[2];
			itsMagnetoelasticField[1] = 2*B1*My*Deformation[1] + (B1*B1/E1-B2*B2/E4)*My*Mz*Mz + B2*Mx*Deformation[2];
			itsMagnetoelasticField[2] = -2*(B1*B1/(6*E1) + E2/E1*B1*(Deformation[0]+Deformation[1]))*Mz + (B1*B1/E1-B2*B2/E4)*(Mx*Mx+My*My)*Mz;
