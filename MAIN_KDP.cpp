...
int main()
{
	...
	FerroelasticSubstrate FS = FerroelasticSubstrate(100, 100, T, 3);
	...
	for(int droga = 1; droga < ilosc_drog; droga++)
	{
		...
		while(T >= 50 && T <= 125)
		{
			plik.open("KDP_wyniki_phi0_theta0_N100_Ds90_L1-8_H8000.txt", std::ofstream::app);
			...
			if(T == 125 && relaxed == 0) Hx = 0; else Hx = 8000;
			...
			if(T == 50 && droga > ilosc_drog-2)
			{
				...
				cout << "eksportuje 50... \n";
			} else if(T == 125 && droga > ilosc_drog-2)
			{
				ML.exportConfiguration(0);
				cout << "eksportuje 125... \n";
			}

			if(T > 100) T += pow(-1,droga)*5; else T += pow(-1,droga)*10;
